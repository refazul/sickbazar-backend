module.exports = {
    GroupMutations: {
        createGroup: async (_, { input }, { dataSources }) => {
            const group = await dataSources.groupAPI.createGroup(input);
            return {
        ...
        updateGroup: async (_, { groupID, input }, { dataSources }) => {
            const group = await dataSources.groupAPI.updateGroup(groupID, input);
            return {
                success: group ? "yes" : "no",
    ...
    GroupQueries: {
        ...
        readGroups: async (_, { title }, { dataSources }) => {
            const groups = await dataSources.groupAPI.readGroups(title);
            return groups;
        }
    }
}